...
export const rpc: Rpc = {
  ...
  /* Browser preview can't reach the rig filesystem. Reject with a clear
     "not in this host" error so FilesView's error pane explains why. */
  fileRead: (_rigId: string, path: string, _range?) =>
    Promise.reject<never>(
      new Error(
        `Browser preview: cannot read ${path} — file.read needs the Tauri host.`,
      ),
    ),
  ...
  agent: {
    ...
    history: {
      list: (_rigId: string) => empty<WireSessionHistoryRow[]>([]),
      reindex: (_rigId: string, sessionId: SessionId) =>
        Promise.reject<never>(
          new Error(
            `Browser preview: cannot reindex ${sessionId} — run under Tauri.`,
          ),
        ),
    },
  },
