...
export interface Ticket {
  ...
  gotchas?: string[];
  verify?: string[];
  /* `@arch:see(path)` references on the work-item — rig-relative paths
     (typically `.yah/arch/authored/<rel>.md|.mmd`). Surfaced as click-
     through chips in the expanded card; the App router routes them via
     `yah://arch/doc/<rel>`. */
  seeAlso?: string[];
  file: string;
  line: number;
