...
const AGENT_PROVIDERS: AgentProviderDef[] = [
  ...
  },
  {
    id: "openai",
    name: "OpenAI",
    costTier: "API key — pay-per-token · Codex CLI login separate",
    description:
      "ChatGPT Plus/Pro does not include API access. Paste a separately-billed key from platform.openai.com/api-keys for yah's direct OpenAI runner, or launch Codex CLI's Sign in with ChatGPT flow so the local codex binary can use your account.",
    helpUrl: "https://platform.openai.com/api-keys",
    unconfiguredLabel: "Not configured",
    footnote:
      "Codex browser login writes OAuth credentials to ~/.codex/auth.json. Those are not sk-… API keys and are not accepted by api.openai.com; yah needs a Codex CLI subprocess runner for that subscription path.",
...
interface AgentProvidersPanelProps {
  ...
  heading?: string;
  subheading?: string;
  /* Optional tab-switch hook used for login flows that should land the
     user in the Terminal tab after creating a local session. */
  onOpenTerminalTab?: () => void;
  onClose?: () => void;
  terminalRigId?: string | null;
}
...
function AgentProviderCard({
  ...
  async function importCodexOpenAI({
    quiet = false,
  } = {}): Promise<"imported" | "missing" | "error"> {
    try {
      const imported = await api.importCodexOpenAi();
      if (imported && !quiet) {
        setTestResult({
          ok: true,
          detail: "Imported Codex login into yah's OpenAI keychain slot.",
        });
      }
      return imported ? "imported" : "missing";
    } catch (err) {
      if (!quiet) {
        setTestResult({
          ok: false,
  ...
  async function pollCodexOpenAIImport() {
    const deadline = Date.now() + 120_000;
    while (Date.now() < deadline) {
      const result = await importCodexOpenAI({ quiet: true });
  ...
  async function signInWithOpenAI() {
    setTestResult(null);
    const importResult = await importCodexOpenAI();
    if (importResult !== "missing") return;
    try {
      const sessionId = await terminalStore.openLocal(
        {
          label: "OpenAI login",
        },
        {
          rigId: terminalRigId ?? null,
        },
      );
      onOpenTerminalTab?.();
      onClose?.();
      setTestResult({
        ok: true,
        detail:
          "OpenAI login terminal opened — switching to Terminal while Codex starts.",
      });
      const backendId = await waitForTerminalBackendId(sessionId);
      if (!backendId) {
        setTestResult({
          ok: false,
          error:
            "Opened a terminal session, but the shell did not become ready in time.",
        });
        return;
      }
      await sendTerminalCommand(backendId, "codex login\n");
      void pollCodexOpenAIImport();
    } catch (err) {
      setTestResult({
        ok: false,
        error: err instanceof Error ? err.message : String(err),
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {provider.id === "openai" && !stored && !adding && (
            <button
              onClick={() => void signInWithOpenAI()}
              className="rounded border border-ink-3/45 bg-paper-2 px-2 py-1 text-[11px] text-ink-2 hover:bg-vellum/55"
            >
              Codex login
            </button>
          )}
...
async function waitForTerminalBackendId(
  sessionId: string,
  timeoutMs = 5000,
): Promise<string | null> {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const session = terminalStore.get(sessionId);
...
async function sendTerminalCommand(
  ...
  await getEnv().then((env) => env.rpc.terminal.input(backendId, btoa(binary)));
}
