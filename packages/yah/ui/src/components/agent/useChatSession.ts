...
/* Settle window before the first auto-index fires. Each `turn_started`
   resets the timer, so a refining follow-up extends the window
   naturally. After it fires once, the session is index-stable until
   the user clicks the re-index button — by design, refactors should
   never shift state out from under the operator. */
const AUTO_INDEX_SETTLE_MS = 5_000;
...
export interface UseChatSessionApi {
  ...
  events: SessionEvent[];
  error: string | null;
  /* Bumps once per session when the post-Q+A auto-index lands. The
     parent uses it as a refresh key for the SessionHistory rail so the
     row's title appears without the operator clicking anything. */
...
export function useChatSession(): UseChatSessionApi {
  ...
  /* Auto-index plumbing. The timer arms on `turn_ended`, resets on the
     next `turn_started` (refinement is in progress, don't index a
     half-cooked exchange), and disarms permanently after the first
     successful index — refactors after that are strictly user-click. */
  const autoIndexFiredRef = useRef<boolean>(false);
  const autoIndexTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [lastIndexedAt, setLastIndexedAt] = useState<number | null>(null);

  const cancelAutoIndex = useCallback(() => {
    if (autoIndexTimerRef.current != null) {
      clearTimeout(autoIndexTimerRef.current);
      autoIndexTimerRef.current = null;
    }
  }, []);

  const armAutoIndex = useCallback(() => {
    if (autoIndexFiredRef.current) return;
    cancelAutoIndex();
    autoIndexTimerRef.current = setTimeout(() => {
      ...
      if (!sid || !rid) return;
      autoIndexFiredRef.current = true;
      void (async () => {
        try {
          const env = await getEnv();
          await env.rpc.agent.history.reindex(rid, sid);
          setLastIndexedAt(Date.now());
        } catch (e) {
          /* Auto-index is best-effort — the row stays unindexed and
             the operator can hit the [index] button. Surface only on
             the WebKit inspector so a routine miss doesn't paint the
             error banner over an otherwise-fine chat. */
          console.warn("[auto-index]", sid, e);
          /* Don't latch fired=true on failure: a later turn_ended
             gets to retry. The cap is "one successful auto-index
             per session", not "one attempt". */
          autoIndexFiredRef.current = false;
        }
      })();
  ...
  function applyEvent(ev: WireRigAgentEvent) {
    switch (ev.kind) {
      ...
      case "turn_started": {
        ...
        setStatus("streaming");
        // Refinement in progress: cancel the pending auto-index. The
        // next turn_ended will re-arm it.
        cancelAutoIndex();
        return;
      }
      case "message_delta": {
  ...
  useEffect(() => () => cancelAutoIndex(), [cancelAutoIndex]);
