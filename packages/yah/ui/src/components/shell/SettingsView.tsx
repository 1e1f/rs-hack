...
export type SettingsSection = "general" | "agents" | "api-keys" | "identities";

interface SettingsViewProps {
  onOpenTerminalTab?: () => void;
  /** Initial section. Defaults to general; contextual nudges (e.g. Infra
   *  empty state) pass "api-keys" so the user lands on the right pane. */
...
const SECTIONS: SectionDef[] = [
  { id: "general", label: "General", hint: "app-wide preferences" },
  { id: "agents", label: "Agents", hint: "providers · approval rules" },
  { id: "api-keys", label: "API Keys", hint: "provider tokens" },
  { id: "identities", label: "Identities", hint: "ssh keys · authorizations" },
];

/* Reserved nav rows for upcoming surfaces. Rendered disabled with a hint so
   the future shape of the rail is visible to operators today. Each lands as
...
/* Settings tab pane. Mirrors the SessionList/ChatPane two-column shape used
   by the Agent and Terminal tabs — left aside enumerates "sub-tabs" (one per
   section), main panel renders the active section. Renders inside the App's
   <main>, so TitleBar + TabStrip stay visible above; users leave Settings by
...
export function SettingsView({
  ...
  // Keep the active section in sync when the gear/contextual nudge re-routes
  // (e.g. Infra → "Configure Hetzner" sets initialSection="api-keys" while
  ...
  return (
    <div className="flex h-full min-h-0">
      <aside className="flex w-[260px] shrink-0 flex-col border-r border-rule/60 bg-paper-2/50">
        <div className="border-b border-rule/60 px-3.5 pt-3 pb-2">
        ...
        <div className="flex flex-1 flex-col gap-1 overflow-y-auto p-2">
          ...
          <div className="mt-2 mb-1 px-2 text-[10px] uppercase tracking-wider text-ink-3/70">
            Coming soon
          </div>
          {FUTURE_SLOTS.map((slot) => (
            <div
              key={slot.label}
              className="flex items-center justify-between rounded border border-dashed border-rule/40 bg-vellum/30 px-2.5 py-1.5 text-[12px] text-ink-3/60"
            >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="mx-auto flex w-full max-w-[1040px] flex-1 overflow-y-auto px-8 py-7">
            ...
            {section === "agents" && (
              <div className="flex flex-col gap-6">
                <AgentProvidersPanel
                  ...
                  terminalRigId={activeRigId ?? null}
                />
