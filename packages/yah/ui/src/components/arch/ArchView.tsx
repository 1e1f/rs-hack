...
interface ArchViewProps {
  ...
  /* Authored arch-doc selection — `null` = JIT graph. Controlled by App
     so a yah://arch/doc/... click on a ticket card can flip the canvas
     from anywhere; the per-rig reset still fires here so a rig switch
     drops the prior selection. */
  authoredMmd: string | null;
  onAuthoredMmdChange: (s: string | null) => void;
...
export function ArchView({
  ...
  /* Per-rig reset — the previous rig's authored selection isn't
     meaningful for the new rig (different sandbox). Stays in this view
     so the App-side router doesn't have to know which rig the path
     belongs to; the rig swap clears it. */
  useEffect(() => {
    onAuthoredMmdChange(null);
  ...
  return (
    <div className="flex h-full">
      <ArchToolbar
        ...
        onAuthoredMmdChange={onAuthoredMmdChange}
      />
