...
//! @yah:next("Define Runner trait in yah-kg-daemon (or new yah-runner crate): start(prelude, ticket_id) -> SessionId; send(session_id, msg) -> Stream<AgentEvent>; stop(session_id). Use yah_kg::agent::{SessionId, AgentEvent} as the contract.")
