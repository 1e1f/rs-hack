...
3. **Check for architecture docs**: If you created or modified any markdown files in `.yah/arch/authored/`, make sure the ticket has an `@arch:see(...)` linking to them. If not, add one:

   ```rust
   //! @arch:see(.yah/arch/authored/path/to/your_doc.md)
   ```
