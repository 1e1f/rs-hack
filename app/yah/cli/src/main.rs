...
//!
//!
//! @arch:see(.yah/arch/authored/agent-skill-cli-surface.md)
//! @arch:see(.yah/arch/authored/yah-roadmap-2026Q2.md)
//!
//! @arch:see(.yah/arch/authored/yahk-cli-binding.md)
//!
//!
//!
//!
...
//!
//! @yah:ticket(R032-T3, "yah agent run: headless CLI driving runner + KeysStore (T3 phase 1)")
//! @yah:status(handoff)
//! @yah:assignee(agent:claude)
//! @yah:parent(R032)
//! @yah:handoff("Phase 1 done. yah agent run --provider {ollama,openai} streams a turn end-to-end. Live-verified against ollama (llama3.1:8b) on localhost:11434 — 'in 5 words: what is rust?' → 'Rust is iron oxidation over time.' with TurnEnded(stop_reason=stop). Anthropic errors loud (path still in desktop). OpenAI reads from T1's KeysStore. Phase 2 (yah-agentd) is now mechanical: same OpenAiCompatRunner construction wrapped in a Unix-socket JSON-RPC server, with the SessionEventSink (T2) plugged into a notification pusher instead of stdout.")
//! @yah:next("Phase 2: new crate yah-agentd. Listen on $XDG_RUNTIME_DIR/yah-agentd.sock. JSON-RPC: agent.start_session, agent.send_message, agent.list_sessions, agent.stop. Notifications: agent.event carrying AgentEvent.")
//! @yah:next("Phase 2: systemd user unit at app/yah/cli/resources/yah-agentd.service + deploy-remote.sh --with-agentd flag (scp unit, systemctl --user enable+start, loginctl enable-linger).")
//! @yah:verify("yah agent run --provider ollama --model llama3.1:8b 'say hi' (needs ollama serve on loopback)")
...
enum Commands {
    ...
    ///
    /// Speaks the same `rpc::*` shapes the Tauri host serves
    ...
    /// Run an agent session headlessly (R032-T3 phase 1).
    ///
    /// Drives `runner`'s OpenAI-compat backend with credentials from
    /// `yah keys`. Anthropic-native still lives in the desktop host.
    #[command(subcommand)]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Agent(agent_cmd) => {
            agent::handle_agent_command(agent_cmd)?;
        }
    }
            ...
            if let Some(ticket) = board.get(&id) {
                if prompt {
                    if let Some(rendered) =
                        board.to_prompt(&ticket.id, kg::prompt::PromptMode::Pickup)
                    {
                        println!("{}", rendered);
                    }
                    return Ok(());
...
fn handle_move(args: MoveArgs) -> Result<()> {
    ...
    // sub-tickets — has its column derived (`kg::board::apply_derived_relay_fields`).
