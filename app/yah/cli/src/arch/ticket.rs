//! @arch:layer(arch)
//! @arch:role(ticket)
//! @arch:see(.yah/arch/authored/hack-board.md)
//!
...
pub struct Ticket {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub think: Option<kg::anno::ThinkBudget>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub engine: Option<kg::anno::EngineRef>,
    ...
    /// `@yah:parent(self.id)`. Computed by [`kg::board::Board`] and
...
pub struct TicketBoard {
    ...
    /// shared renderer in [`kg::prompt`]. Populated by
...
impl TicketBoard {
    ...
    /// [`kg::board::Board`] for the cross-anchor recompute (epic
    ...
    pub fn from_annotations(annotations: &[ArchAnnotation]) -> Self {
        use kg::anno::WorkItemType as KgItemType;
        use kg::board::Board;
        use kg::ids::NodeId;
        use kg::kind::Lang;
        use rpc::WorkItem;
        ...
        for (id, mut partials) in partials_by_id {
            ...
            let anchors: Vec<rpc::WorkItemAnchor> = partials
                .iter()
    ...
    /// Delegates to [`kg::prompt::render`] — the same renderer the
    ...
    pub fn to_prompt(
        &self,
        id: &str,
        mode: kg::prompt::PromptMode,
    ) -> Option<String> {
        kg::prompt::render(&self.kg_board, id, mode)
    }
...
impl Ticket {
    ...
    /// CLI shim: delegates to [`kg::prompt::render`] (the source-of-truth
    /// renderer the daemon's `arch.ticket_prompt` RPC also uses) by translating
    /// `self + ctx` into a one-off [`kg::board::Board`]. Both paths produce
    ...
    /// [`kg::prompt`] for the rendering logic.
    ///
    ...
    pub fn to_prompt_with_ctx(&self, ctx: &PromptContext) -> String {
        ...
        kg::prompt::render(&board, &self.id, kg::prompt::PromptMode::Pickup)
            .unwrap_or_default()
    }
...
struct PartialTicket {
    ...
    think: Option<kg::anno::ThinkBudget>,
    engine: Option<kg::anno::EngineRef>,
}
...
fn partial_to_anchor(id: &str, p: &PartialTicket) -> rpc::WorkItemAnchor {
    let anno = kg::anno::WorkItemAnno {
        id: id.to_string(),
...
fn to_kg_status(s: &TicketStatus) -> kg::anno::TicketStatus {
    use kg::anno::TicketStatus as Kg;
    match s {
...
/// renderer in [`kg::prompt`] consumes. Used by the
/// [`Ticket::to_prompt_with_ctx`] shim to feed its renderer a
/// `Board::from_work_items`-shaped view.
fn ticket_to_work_item(t: &Ticket) -> rpc::WorkItem {
    use kg::anno::WorkItemType;
    use kg::ids::NodeId;
    use kg::kind::Lang;
    ...
    let mut anchors: Vec<rpc::WorkItemAnchor> = t
        .files
        .iter()
        .map(|loc| {
    ...
    if anchors.is_empty() {
        ...
        anchors.push(rpc::WorkItemAnchor {
            node: NodeId::compute(
                Lang::Rust,
...
/// Build a one-off [`kg::board::Board`] from `t` and its
/// `PromptContext` neighbours so [`kg::prompt::render`] can render
/// without needing a full workspace scan.
fn build_one_off_kg_board(t: &Ticket, ctx: &PromptContext) -> kg::board::Board {
    use kg::anno::WorkItemType;

    let mut relays: Vec<rpc::WorkItem> = Vec::new();
    let mut tickets: Vec<rpc::WorkItem> = Vec::new();
    ...
    kg::board::Board::from_work_items(relays, tickets)
}

fn from_kg_status(s: kg::anno::TicketStatus) -> TicketStatus {
    use kg::anno::TicketStatus as Kg;
    match s {
...
//! @yah:phase(P2)
//! @arch:see(.yah/arch/authored/vivarium/voice_allocation.md)
...
"#;
        ...
        assert_eq!(ticket.think, Some(kg::anno::ThinkBudget::Deep));
...
"#;
        ...
        assert_eq!(
            ticket.think,
            Some(kg::anno::ThinkBudget::Budget { tokens: 8192 })
        );
    }
...
//! @arch:see(.yah/arch/authored/vivarium/PROCESS_BLOCK_UNIFICATION.md)
...
"#;
        ...
        let prompt = board
            .to_prompt("R200", kg::prompt::PromptMode::Pickup)
