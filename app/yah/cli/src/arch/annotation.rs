...
impl ArchKind {
    ...
    pub fn parse(key: &str, value: &str) -> Self {
        match key {
            ...
            "think" => match kg::anno::ThinkBudget::parse(value.trim_matches('"')) {
                Ok(t) => Self::Think(t),
                Err(_) => Self::Unknown {
                    key: key.to_string(),
                    value: value.to_string(),
            ...
            "engine" => match kg::anno::EngineRef::parse(value.trim_matches('"')) {
                Ok(e) => Self::Engine(e),
                Err(_) => Self::Unknown {
                    key: key.to_string(),
                    value: value.to_string(),
