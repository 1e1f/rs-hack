...
fn init_tracing() {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,desktop=debug,runner=debug,kg_daemon=info"));
...
/// macOS GUI-launched apps (Finder / Dock / `open yah.app`) inherit a
/// stripped PATH (`/usr/bin:/bin:/usr/sbin:/sbin`), so subprocess
/// spawns of `claude`, `yah`, `git`, `bun`, etc. installed under
/// `/opt/homebrew/bin`, `/usr/local/bin`, `~/.bun/bin`, `~/.npm-global/bin`
/// fail with ENOENT. Replace PATH with the user's login-shell PATH so
/// every Command::new("claude") downstream resolves the way it does
/// from a terminal. No-op when launched from a terminal (the shell
/// already populated PATH for us) and on non-macOS.
#[cfg(target_os = "macos")]
fn fix_path_from_login_shell() {
    use std::process::Command;
    let current = std::env::var("PATH").unwrap_or_default();
    let only_system = !current.is_empty()
        && current
            .split(':')
            .all(|p| matches!(p, "/usr/bin" | "/bin" | "/usr/sbin" | "/sbin" | ""));
    if !only_system {
        return;
    }
    let shell = std::env::var("SHELL").unwrap_or_else(|_| "/bin/zsh".into());
    let out = Command::new(&shell)
        .args(["-ilc", "/bin/echo -n $PATH"])
        .output();
    ...
    tracing::info!(shell = %shell, new_path_len = new_path.len(), "PATH augmented from login shell for GUI-launched bundle");
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            api_keys::api_key_import_codex_openai,
            ...
            session_history::agent_session_history_list,
            session_history::agent_session_history_reindex,
...
async fn boot_registry(state: AppState, app_handle: AppHandle) {
    ...
    match identities::migrate_rigs_keypath_to_identity_id() {
        ...
        Ok(n) => tracing::info!(
            migrated = n,
            "rigs.json keyPath → identityId migration complete"
        ),
