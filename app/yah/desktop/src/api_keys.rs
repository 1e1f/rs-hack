...
//! in `.yah/arch/authored/settings-api-keys.md` requires that tokens never
...
//! @arch:see(.yah/arch/authored/settings-api-keys.md)
...
fn save_vault(map: &HashMap<String, String>) -> Result<(), ApiKeyError> {
    if map.is_empty() {
        ...
        let json = serde_json::to_string(map).expect("HashMap<String, String> always serializes");
...
#[tauri::command]
pub async fn api_key_import_codex_openai() -> Result<bool, String> {
    let path = codex_auth_path()
        .ok_or_else(|| "could not resolve home directory for ~/.codex/auth.json".to_string())?;
    ...
    if let Some(api_key) = api_key {
        set("openai", api_key).map_err(|err| err.to_string())?;
        return Ok(true);
    }
    if json
        .get("tokens")
    ...
    {
        return Err(format!(
            "{} contains Codex ChatGPT OAuth credentials, not an OpenAI API key. \
             yah cannot use that token with api.openai.com; paste a platform API key \
             or add a Codex CLI subprocess runner for the ChatGPT-plan path.",
            path.display(),
        ));
    }
    return Err(format!(
        "{} does not contain an OpenAI API key; run `codex login --api-key <key>` \
         or paste a platform key in yah.",
        path.display(),
    ));
}
...
mod tests {
    ...
    fn fold_legacy_inserts_new_entries_and_deletes_originals() {
        ...
        assert!(
            !vault.contains_key("cloudflare"),
            "legacy didn't have cloudflare"
        );
