...
use yah_runner::ToolRegistry;
use yah_tauri::agent_approval::{ApprovalChoice, ApprovalRouter, StaticApprovalRouter};
use yah_tauri::agent_tools::{KgToolRegistry, ToolContext};
use yah_tauri::state::RigId;
...
async fn edit_file_round_trips_through_reindex_with_agent_edit_reason() {
    ...
    let ev = wait_for(
        &mut rx,
        |e| matches!(e, ArchEvent::IndexStarted { reason: IndexReason::AgentEdit, .. }),
...
async fn write_arch_doc_creates_file_under_authored_sandbox() {
    ...
    assert!(outcome.ok, "{outcome:?}");
    let body =
        std::fs::read_to_string(tmp.path().join(".yah/arch/authored/topology.mmd")).unwrap();
...
async fn writer_chain_leaves_rig_in_consistent_state() {
    ...
    let mmd =
        std::fs::read_to_string(tmp.path().join(".yah/arch/authored/chain.mmd")).unwrap();
...
async fn yah_add_envelope_round_trips_when_yah_binary_present() {
    ...
    if exit_code == 0 && outcome.result["touched"].as_array().map(|a| !a.is_empty()).unwrap_or(false)
    {
        let body = std::fs::read_to_string(tmp.path().join("src/lib.rs")).unwrap();
        assert!(body.contains("label"), "yah_add succeeded but file unchanged: {body}");
