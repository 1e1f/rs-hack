...
//!
//! See `architecture/yah-identities.md` for the full design including
...
fn yah_home() -> Result<PathBuf, IdentityError> {
    ...
    let home = std::env::var_os("HOME").map(PathBuf::from).ok_or(IdentityError::NoHomeDir)?;
    Ok(home.join(".yah"))
}
...
pub fn create(name: &str) -> Result<Identity, IdentityError> {
    ...
    let identity = Identity {
        ...
        source: IdentitySource::YahGenerated { private_key_path: private_path },
        authorized_at: vec![],
        created_at: now_ms(),
        last_used_at: None,
    };
...
/// than a hard error.
async fn fetch_github_authorizations(
) -> Result<Vec<(String, Authorization)>, GithubProbeError> {
...
pub async fn probe_all() -> Result<ProbeReport, IdentityError> {
    ...
    let identity_ids: HashSet<String> =
        file.identities.iter().map(|i| i.id.clone()).collect();
    let now = now_ms();
...
/// `last_used_at`.
pub async fn authorize_hetzner(
    id: &str,
    name: &str,
...
pub async fn deauthorize_hetzner(id: &str) -> Result<bool, String> {
    ...
    let key_id = identity.authorized_at.iter().find_map(|a| match a {
        Authorization::Hetzner { key_id_in_hetzner, .. } => Some(*key_id_in_hetzner as u64),
        _ => None,
    });
...
mod tests {
    ...
    fn validate_rejects_paths_and_unicode() {
        for n in ["", "with space", "../escape", "slash/key", "with.dot", "ñame"] {
    ...
    fn create_round_trips_through_list_and_remove_deletes_keyfile() {
        with_temp_yah_home(|home| {
            ...
            assert!(matches!(create("yah_test"), Err(IdentityError::AlreadyExists(_, _))));
    ...
    fn import_does_not_copy_private_key_bytes() {
        with_temp_yah_home(|home| {
            ...
            fs::write(&private_path, key.to_openssh(LineEnding::LF).unwrap().as_bytes()).unwrap();
            ...
            assert!(remove(&id.id).expect("remove"));
            assert!(private_path.is_file(), "imported private key file must remain after remove");
            assert!(public_path.is_file(), "imported public key file must remain after remove");
        });
    }
    ...
    fn probe_local_picks_up_yah_keys_and_ssh_keys_then_no_ops() {
        with_temp_yah_home(|home| {
            ...
            fs::write(&yah_priv, yah_key.to_openssh(LineEnding::LF).unwrap().as_bytes()).unwrap();
            fs::write(
                &yah_pub,
            ...
            assert!(matches!(yah_entry.source, IdentitySource::YahGenerated { .. }));
    ...
    fn write_authorization_replaces_same_provider_and_preserves_others() {
        with_temp_yah_home(|_home| {
            ...
            assert!(after.authorized_at.iter().any(|a| matches!(
                a,
                Authorization::Hetzner { key_id_in_hetzner: 222, .. }
            )));
            assert!(after.authorized_at.iter().any(|a| matches!(
                a,
                Authorization::Github { key_id: 999, .. }
            )));
