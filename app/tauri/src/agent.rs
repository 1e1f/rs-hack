...
//! matrix (`architecture/yah-agent-runtime.md`'s "Composable runtime
...
//! @yah:phase(P1)
//! @yah:parent(R028)
//! @yah:gotcha("Scope reshape on 2026-04-28: this ticket originally also owned the Pro/Max OAuth flow inside a Tauri webview. That work is now policy-blocked — Anthropic's 2026-04-04 TOS change bans consumer OAuth in third-party tools (and explicitly names the Agent SDK). Terminology locked in: this crate serves the HA-family presets — `anthropic` (HAk, API key), `crab` (HAo, OAuth — kept as a disabled-by-default slot), and `mcp-connect` (HAk+M / HAo+M, MCP-connector beta). The cost-controlling subscription path moved to the `claude` (PVd) preset in R028-F8 / R028-F9. See architecture/yah-agent-runtime.md 'Composable runtime matrix' for the canonical preset table + axis encoding (Transport · Protocol · Auth).")
...
//! @yah:next("R031-F5 (P5): land KgToolRegistry::execute_gated(call, &ctx) at the host layer between yah_runner::ToolRegistry::execute and Tool::execute. Approval rules + bash arg parser + settings UI per the F5 ticket. Once that lands, swap agent::start_runner_session's standard_read_only -> with_experimental_writers behind an opt-in settings flag.")
...
//! @yah:phase(P5)
//! @yah:parent(R031)
//! @yah:gotcha("Gate placement is load-bearing: the approval check lives at the HOST layer (inside KgToolRegistry, between the runner-shaped ToolRegistry::execute and Tool::execute), NOT inside yah_runner::ToolRegistry::execute. R028's Anthropic path parses tool_use blocks in run_anthropic_turn directly — it never goes through the yah_runner trait. KgToolRegistry::execute_gated is the single gate; ToolRegistry::execute is a thin shim. KgToolRegistry exposes tool(name)/tools() borrows so the Anthropic loop can iterate schemas + dispatch without round-tripping through the runner trait. See architecture/agent-tool-calls.md 'Approval gate placement (load-bearing)'.")
...
//! @yah:phase(P3)
//! @yah:parent(R028)
//! @yah:next("New crate yah-mcp wrapping yah_tauri::agent_tools::KgToolRegistry. Two transports: stdio (launched by the claude (PVd) preset's subprocess on its own stdin/stdout bus) and TCP listener (for the mcp-connect (+M modifier) preset when Anthropic's MCP-connector beta GAs).")
...
//! @arch:see(architecture/agent-eval-rig.md)
//! @arch:see(architecture/agent-tool-calls.md)
//!
...
use yah_kg::agent::{AgentEvent, Message, Role, SessionId, TurnUsage};
use yah_kg::anno::{EngineRef, ThinkBudget};
use yah_kg::prelude::Prelude;
use yah_kg::rpc::AssemblePreludeParams;
use yah_kg_daemon::KgService;
use yah_runner::{
    list_openai_compat_models, mint_session_id, tap_stream, OpenAiCompatConfig,
    OpenAiCompatRunner, Runner, SessionEventSink, SessionStore, ToolRegistry,
};
...
/// personal-use call — see `architecture/yah-agent-runtime.md`'s
...
/// `architecture/yah-claude-shape-capture.md`). When present, [`apply`]
...
fn resolve_anthropic_auth() -> Result<AnthropicAuth, String> {
    ...
    let api_key = api_keys::get(ANTHROPIC_PROVIDER)
        .map_err(|e| format!("keychain read failed: {e}"))?;
    match api_key {
...
/// `architecture/yah-agent-runtime.md`.
...
impl AgentSessions {
    ...
    pub async fn remove_process(
        &self,
        id: &SessionId,
    ) -> Option<Arc<Mutex<ProcessSession>>> {
...
pub async fn agent_start_session(
    ...
    match engine.provider.as_str() {
        ...
        other => Err(format!(
            "unsupported engine provider '{other}' — known providers: claude, anthropic, crab, claude-cli, openai, ollama (see architecture/yah-agent-runtime.md)",
        )),
    }
}
...
async fn start_runner_session(
    ...
    let registry_dyn: Arc<dyn ToolRegistry> = Arc::clone(&registry_concrete) as Arc<dyn ToolRegistry>;
...
fn build_chat_prelude(rig_name: &str, engine: Option<EngineRef>) -> Prelude {
    use yah_kg::prelude::{
        output_conventions_section, CacheControl, CacheTtl, PreludeSection,
        PreludeSectionKind,
    };
...
fn render_sections(sections: &[yah_kg::prelude::PreludeSection]) -> String {
    ...
    match provider.as_str() {
        "openai" => {
            let key = api_keys::get(OPENAI_PROVIDER)
                .map_err(|e| format!("keychain read failed: {e}"))?
                .ok_or_else(|| {
                    "no OpenAI API key in keychain — set one in Settings → Agents"
                        .to_string()
                })?;
        ...
        "ollama" => {
            ...
            let key = api_keys::get(OLLAMA_PROVIDER)
                .map_err(|e| format!("keychain read failed: {e}"))?;
...
async fn run_anthropic_turn(
    ...
    let (session_id, rig_id, body) = {
        ...
        (s.id.clone(), s.rig_id.clone(), build_anthropic_body(&s, auth))
    };
...
/// Tauri-host implementation of [`yah_runner::SessionEventSink`]: wraps
...
impl<'a> SessionEventSink for TauriRigSink<'a> {
    fn emit(&self, event: &AgentEvent) {
        let payload = RigAgentEvent { rig_id: self.rig_id, event };
