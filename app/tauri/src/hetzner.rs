...
//! `architecture/settings-api-keys.md` for the threat-model rationale
...
pub async fn list_servers() -> Result<Vec<HetznerServer>, HetznerError> {
    ...
    Ok(parsed.servers.into_iter().map(HetznerServer::from).collect())
}
...
pub async fn list_ssh_keys() -> Result<Vec<HetznerSshKey>, HetznerError> {
    ...
    Ok(parsed.ssh_keys.into_iter().map(HetznerSshKey::from).collect())
}
...
pub async fn list_locations() -> Result<Vec<HetznerLocation>, HetznerError> {
    ...
    Ok(parsed.locations.into_iter().map(HetznerLocation::from).collect())
}
