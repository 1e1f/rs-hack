...
//! abstract [`yah_runner::ToolRegistry`] trait the runner consumes.
//!
//! ## Why two traits
//!
//! The runner sees [`yah_runner::ToolRegistry`] — `schemas() + execute(name,
...
//! [`yah_kg_daemon::KgService::read_authored_file`] — canonicalize the
...
//! after the mutation succeeds so [`yah_kg::event::IndexReason::AgentEdit`]
...
use yah_kg::rpc::{Direction, LookupParams, NeighborsParams, SubgraphParams};
use yah_kg_daemon::KgService;
use yah_runner::{ToolOutcome, ToolRegistry, ToolSchema};
...
/// implements. Sister to [`yah_runner::ToolRegistry`] which is what the
...
///
/// Implements [`yah_runner::ToolRegistry`] so the runner sees the
...
impl KgToolRegistry {
    ...
    /// context (see architecture/agent-tool-calls.md "Approval gate
    /// placement").
    ///
    ...
    pub async fn execute_gated(
        ...
        match self.gate.decide(&pending) {
            ...
            ApprovalDecision::NeedsPrompt { reason } => {
                match self.prompt_for_approval(name, &args, parsed_bash.as_ref()).await {
...
fn smell_signal(name: &str, result: &Value, ok: bool) -> String {
    ...
    match name {
        "read_file" => {
            ...
            let trunc = result.get("truncated").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        "list_dir" => {
            let entries = result.get("entries").and_then(|v| v.as_array()).map(|a| a.len());
            entries.map(|n| format!("{n} entries")).unwrap_or_default()
        }
        "grep" => {
            let hits = result.get("hits").and_then(|v| v.as_array()).map(|a| a.len());
            let trunc = result.get("truncated").and_then(|v| v.as_bool()).unwrap_or(false);
            match (hits, trunc) {
...
/// [`yah_kg_daemon::KgService::read_authored_file`]'s pattern. Returns
...
fn resolve_in_sandbox(rig_root: &Path, rel: &str) -> Result<PathBuf, ToolError> {
    ...
    let candidate_canon = candidate.canonicalize().map_err(|e| {
        ToolError::Operation(format!("cannot resolve {rel}: {e}"))
    })?;
...
impl Tool for Grep {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        ...
        let cap = args
            .max_hits
            .unwrap_or(GREP_MAX_HITS)
            .min(GREP_MAX_HITS);

        let mut hits: Vec<Value> = Vec::new();
        let mut truncated = false;

        let walker = walkdir::WalkDir::new(&ctx.rig_root).into_iter().filter_entry(|e| {
...
impl Tool for ArchNode {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        let args: ArchNodeArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::InvalidArgs(e.to_string()))?;
...
impl Tool for ArchNeighbors {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        let args: ArchNeighborsArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::InvalidArgs(e.to_string()))?;
        let id = NodeId::from_hex(&args.id)
            .map_err(|e| ToolError::InvalidArgs(format!("bad node id: {e}")))?;
...
impl Tool for ArchSubgraph {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        let args: ArchSubgraphArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::InvalidArgs(e.to_string()))?;
...
impl Tool for ArchLookup {
    ...
    fn schema(&self) -> ToolSchema {
        ToolSchema {
            name: self.name().into(),
            description: "Resolve a file:line to the innermost knowledge-graph nodes that span it. \
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        let args: ArchLookupArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::InvalidArgs(e.to_string()))?;
        let result = ctx
            .svc
            .lookup(LookupParams {
...
impl Tool for ReadArchDoc {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        let args: ReadArchDocArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::InvalidArgs(e.to_string()))?;
        let result = ctx
            .svc
            .read_authored_file(yah_kg::rpc::ReadAuthoredFileParams {
...
/// sandbox or that [`yah_kg_daemon::is_eligible`]-style filters reject
/// (calling `reindex_path` on `target/` is a no-op anyway).
async fn reindex_glob(
    svc: &KgService,
    rig_root: &Path,
    raw_paths: &str,
) -> Vec<String> {
...
async fn dispatch_yah(
    ...
    let (stdout, stderr, exit_code) =
        run_yah_cli(&ctx.rig_root, subcommand, &cli_args).await?;
...
impl Tool for YahAdd {
    ...
    fn schema(&self) -> ToolSchema {
        ToolSchema {
            name: self.name().into(),
            description:
                "Add a Rust AST node — struct field, enum variant, impl method, derive, \
...
impl Tool for YahRemove {
    ...
    fn schema(&self) -> ToolSchema {
        ToolSchema {
            name: self.name().into(),
            description:
                "Remove a Rust AST node — struct field, enum variant, match arm, doc \
...
impl Tool for YahTransform {
    ...
    fn schema(&self) -> ToolSchema {
        ToolSchema {
            name: self.name().into(),
            description:
                "Comment, remove, or replace any matched AST nodes. Lower-level than \
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        ...
        push_str_flags(
            &args,
            &["paths", "node_type", "action", "name", "content_filter", "with"],
            &mut cli,
        );
        cli.push("--apply".into());
        let paths = opt_str(&args, "paths");
...
impl Tool for WriteArchDoc {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        let args: WriteArchDocArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::InvalidArgs(e.to_string()))?;
        ...
        if let Some(parent) = candidate.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                ToolError::Operation(format!("create {}: {e}", parent.display()))
            })?;
        }
        ...
        let parent_canon = parent.canonicalize().map_err(|e| {
            ToolError::Operation(format!("canonicalize {}: {e}", parent.display()))
        })?;
        ...
        std::fs::write(&final_path, &args.content).map_err(|e| {
            ToolError::Operation(format!("write {}: {e}", final_path.display()))
        })?;
        ...
        let _ = ctx.svc.reindex_path(&final_path, IndexReason::AgentEdit).await;
        Ok(json!({
            "rel_path": args.rel_path,
...
impl Tool for EditFile {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> Result<Value, ToolError> {
        ...
        let content = String::from_utf8(bytes).map_err(|_| {
            ToolError::Operation(format!("{} is not valid UTF-8", args.path))
        })?;
        ...
        std::fs::write(&canon, &updated).map_err(|e| {
            ToolError::Operation(format!("write {}: {e}", args.path))
        })?;
...
fn parse_edge_kinds(
    raw: Option<&[String]>,
) -> Result<Option<Vec<yah_kg::edge::EdgeKind>>, ToolError> {
    ...
    for name in slice {
        ...
        let kind: yah_kg::edge::EdgeKind = serde_json::from_value(json).map_err(|_| {
            ToolError::InvalidArgs(format!(
                "unknown edge kind '{name}' — see EdgeKind in yah_kg::edge"
            ))
        })?;
        out.push(kind);
    }
...
mod tests {
    ...
    async fn write_arch_doc_creates_authored_file_inside_sandbox() {
        ...
        let registry = KgToolRegistry::with_experimental_writers(ctx)
            .with_store(allow_all_writes_store());
        let outcome = registry
            .execute(
                "write_arch_doc",
                json!({
