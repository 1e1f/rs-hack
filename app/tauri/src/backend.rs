...
//!
//! The wire shapes used by both arms come from `yah-kg::rpc` — the
//! contract is the same on both sides, so the renderer is unaware of
//! the backend kind. See `architecture/rig-backend-dispatch.md` for the
//! design rationale.
//!
//! ## Errors
//!
...
use yah_rpc_ssh::{ReindexReasonWire, SshRpcClient};
...
impl RigBackend {
    ...
    pub async fn list_authored_files(
        ...
        match self {
            Self::Local(svc) => svc.list_authored_files(params).await.map_err(|e| e.to_string()),
            Self::Remote(client) => {
                client.list_authored_files(params).await.map_err(|e| e.to_string())
            }
        }
    }
    ...
    pub async fn read_authored_file(
        ...
        match self {
            Self::Local(svc) => svc.read_authored_file(params).await.map_err(|e| e.to_string()),
            Self::Remote(client) => {
                client.read_authored_file(params).await.map_err(|e| e.to_string())
            }
        }
    }
    ...
    pub async fn ticket_prompt(
        ...
        match self {
            ...
            Self::Remote(client) => client.ticket_prompt(params).await.map_err(|e| e.to_string()),
        }
    }
...
impl WalkSummaryDto {
    fn from_walk_summary(s: &yah_kg_store::WalkSummary) -> Self {
        Self {
