...
pub enum TerminalError {
    ...
    NoHomeDir,
    #[error("terminal_open_ssh requires keyPath — yah will not auto-discover keys it didn't create")]
    KeyPathRequired,
...
pub async fn terminal_open_ssh(
    ...
    let user = spec.user.clone().unwrap_or_else(|| DEFAULT_USER.to_string());
    ...
    tauri::async_runtime::spawn(async move {
        run_session(app, sid, user, (host, port), key_path, cols, rows, control_rx).await;
    });
