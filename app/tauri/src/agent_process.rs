...
//! @yah:next("Authoring page architecture/yah-claude-md-generator.md: rule schema, examples, scoping rules, how to test (yah board agent-context --ticket <ID> --format claude prints the rendered CLAUDE.md).")
...
//! @arch:see(architecture/yah-claude-md-generator.md)
//!
...
use yah_kg::agent::{AgentEvent, SessionId, TurnUsage};
use yah_kg::anno::EngineRef;
use yah_kg::prelude::Prelude;
use yah_runner::mint_session_id;
...
pub async fn start_process_session(
    ...
    let mut child = spawn_claude(rig_root, &model)
    ...
    loop {
        ...
        match next {
            Ok(Some(line)) => {
                ...
                let translated = translate_frame(
                    &value,
                    &session_id,
                    &mut turn_open,
                    &mut accumulated,
