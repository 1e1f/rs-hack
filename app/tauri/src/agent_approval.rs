...
//! [`yah_runner::ToolRegistry::execute`] entry point and concrete
...
//!   [`yah_runner::ToolRegistry::execute`], which forwards to
        ...
        match c {
            ...
            '"' | '\'' => {
                ...
                loop {
                    ...
                    if inner == '\\' && quote_char == '"' {
                        let escaped =
                            chars.next().ok_or(BashParseError::TrailingBackslash)?;
                        cur.push(escaped);
                    } else {
                        cur.push(inner);
                    }
...
impl ApprovalGate {
    ...
    pub fn decide(&self, call: &PendingCall) -> ApprovalDecision {
        ...
        ApprovalDecision::NeedsPrompt {
            reason: format!(
                "no approval rule matches write tool '{}'",
                call.tool_name
            ),
        }
    }
...
fn glob_match(pattern: &str, candidate: &str) -> bool {
    fn rec(pat: &[u8], s: &[u8]) -> bool {
        ...
        if pat.starts_with(b"**") {
            ...
            // (including zero).
            let after_sep = if rest.starts_with(b"/") { &rest[1..] } else { rest };
...
/// renderer. Rides on [`yah_kg::agent::AgentEvent::ApprovalRequested`]
...
mod tests {
    ...
    fn parse_bash_treats_post_cmd_eq_as_arg() {
        ...
        assert_eq!(
            r.args,
            vec!["build", "--target=x86_64-unknown-linux-gnu"]
        );
    ...
    fn ruleset_round_trips_through_serde() {
        ...
        rs.push(ApprovalRule::Tool { name: "yah_rename".into() });
        rs.push(ApprovalRule::BashCmdPattern {
            cmd: "cargo".into(),
            args: vec![
                ArgPattern::Exact { value: "build".into() },
                ArgPattern::Any,
            ],
        });
    ...
    fn file_store_round_trips_through_disk() {
        ...
        store.push(ApprovalRule::BashCmdPattern {
            cmd: "cargo".into(),
            args: vec![ArgPattern::Exact { value: "build".into() }],
        });
        ...
        assert!(rules.iter().any(
            |r| matches!(r, ApprovalRule::Tool { name } if name == "yah_rename")
        ));
    ...
    fn file_store_remove_at_handles_out_of_range() {
        let store = FileApprovalStore::load_or_empty(
