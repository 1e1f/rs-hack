...
fn test_extracts_annotations_from_workspace() {
    ...
    let has_core = files.iter().any(|f| f.contains("yah/src/") && !f.contains("/arch/") && !f.contains("/mcp/"));
    let has_mcp = files.iter().any(|f| f.contains("yah/src/mcp/"));
    let has_arch = files.iter().any(|f| f.contains("yah/src/arch/"));

    assert!(has_core, "Missing annotations from yah core modules");
    assert!(has_mcp, "Missing annotations from yah::mcp");
    assert!(has_arch, "Missing annotations from yah::arch");
}
...
fn test_editor_has_multiple_roles() {
    ...
    let editor_node = graph
        .nodes()
        .find(|n| n.file.to_string_lossy().contains("yah/src/editor.rs"));
...
fn test_file_context_editor() {
    let graph = build_graph();
    let ctx = get_file_context(&graph, "yah/src/editor.rs");
    ...
    let md = ctx.to_markdown("yah/src/editor.rs");
...
fn test_depends_on_edges() {
    ...
    let dep_edges: Vec<_> = graph
        .edges()
        .filter(|e| e.kind == yah::arch::graph::EdgeKind::DependsOn)
        .collect();
