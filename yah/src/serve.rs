...
//! the same shapes Tauri commands speak today (`yah_kg::rpc::*`).
//!
//! Wire shape:
...
use yah_kg::event::{ArchEvent, FileEvent, IndexReason};
...
async fn dispatch(method: &str, mut params: Value, svc: &Arc<KgService>) -> Result<Value, RpcError> {
    ...
    match method {
        ...
        // vocabulary across local/remote (see architecture/rig-backend-dispatch.md).
