...
export function App() {
  ...
  /* Settings modal lives at App level so contextual nudges (Infra empty
     state) can open it pre-targeted to a section + provider row. The gear
     button just opens the default General view. */
  const [settingsRequest, setSettingsRequest] = useState<SettingsRequest | null>(
    null,
  );
  const openSettings = useCallback(
    (req: SettingsRequest = { section: "general" }) => setSettingsRequest(req),
    [],
  );
  ...
  /* yah:// link router. Agent output emits `[label](yah://...)` anchors
     ...
     where the click lands. Two shapes today:
       yah://file/<path>[#L<line>]    -> jumpToFile (Arch graph re-root)
       yah://arch/<symbol>            -> Arch graph re-root by symbol id
