...
use yah_runner::{OpenAiCompatConfig, OpenAiCompatRunner, Runner};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(event) = stream.next().await {
        ...
        match event {
            yah_runner::AgentEvent::TurnEnded { .. } => got_turn_ended = true,
            yah_runner::AgentEvent::TurnFailed { message, .. } => {
