...
//!
//! Method shapes mirror the `arch.*` RPC surface in `rpc`.
//!
...
//! @yah:handoff("Extracted yah-rpc as a new workspace crate at crates/yah/rpc/. Moved src/lib.rs from former kg::rpc module. Foundational types that kg internally consumes (Direction, Subgraph, WorkItem, WorkItemAnchor) graduated back into kg proper (Direction → kg::edge, Subgraph → new kg::subgraph, WorkItem/WorkItemAnchor → kg::board) so rpc → kg is a clean one-way dep. rpc re-exports them for callers who want the rpc::Direction/rpc::Subgraph/rpc::WorkItem path. All 11 consumer crates now depend on rpc directly: kg-store / kg-validator / kg-anno / kg-daemon / rpc-ssh / app/yah/cli / app/yah/desktop in [dependencies]; kg-rust / kg-ts / kg-json-yaml in [dev-dependencies]. 46 kg::rpc:: call sites rewrote to rpc:: via sed; one audit-log mention restored to its historical wording (R017-F2 handoff). cargo check --workspace + cargo build -p desktop both green; tests across kg, kg-store, kg-anno, kg-validator, kg-daemon, rpc-ssh, kg-rust, kg-ts, kg-json-yaml all green (no test count regressions). TS-side prose doc comments in packages/yah/ui/src/env/{index,types}.ts updated from yah_kg::rpc:: → rpc::.")
//! @yah:next("R019-F2 (rpc-ssh) currently has both kg + rpc deps; once it stops referring to ArchEvent/etc. via kg, it can drop kg. Optional cleanup, not load-bearing.")
//! @yah:verify("cargo check --workspace")
//! @yah:verify("cargo test -p rpc -p kg -p kg-store -p kg-daemon -p rpc-ssh")
//!
...
//! @yah:next("Optional: when R019-T1 lifts rpc::* into a yah-rpc crate, bump yah-rpc-ssh's dep from yah-kg -> yah-rpc; the import lines in client.rs are the only churn — test infra in session.rs uses serde_json::Value + ArchEvent (which stays in yah-kg::event) so it's unaffected.")
...
use kg::event::{ArchEvent, FileEvent, FileEventKind, IndexReason, IndexScope};
use kg::ids::{NodeFull, NodeId};
use kg::indexer::IndexError;
...
use kg_anno::{apply_pass, AnnotationIndex, TouchedWorkItem, WorkItemType};
use kg_store::{reindex_file, walk_and_index, IndexerRegistry, Store, WalkSummary};
use kg_validator::validate as run_validator;
...
impl KgService {
    ...
    /// app. See [`kg::board::apply_derived_relay_fields`] for the rules.
    pub async fn list_relays(&self, _params: ListRelaysParams) -> ListRelaysResult {
        ...
        let tickets =
            collect_work_items(&store, &anno, WorkItemType::Ticket, rig_root.as_deref());
        kg::board::apply_derived_relay_fields(&mut relays, &tickets);
    ...
    /// becomes zero or more [`kg::validate::Violation`]s — parse and
    ...
    ///
    /// Builds a [`kg::board::Board`] from the daemon's current relay /
    /// ticket state and dispatches to [`kg::prompt::render`] — the
    ...
    pub async fn ticket_prompt(&self, params: TicketPromptParams) -> TicketPromptResult {
        ...
        let board = kg::board::Board::from_work_items(relays, tickets);
        let markdown = kg::prompt::render(&board, &params.id, params.mode);
    ...
    pub async fn assemble_prelude(&self, params: AssemblePreludeParams) -> AssemblePreludeResult {
        ...
        let mut options = kg::prelude::PreludeOptions::default();
        ...
        let inputs = kg::prelude::PreludeInputs {
            ticket_id: &params.id,
            board: &board,
        ...
        AssemblePreludeResult {
            prelude: kg::prelude::assemble(&inputs),
        }
    }
    ...
    /// lives on the daemon (see [`kg::board_mutate::bucket_to_status`])
    ...
    pub async fn move_ticket(
        ...
        let new_content = if after_status != original {
            let now_secs = std::time::SystemTime::now()
                ...
                .map(|d| d.as_secs())
                .unwrap_or(0);
            let now_rfc = kg::timefmt::format_rfc3339(now_secs);
...
fn emit_delta(events: &broadcast::Sender<ArchEvent>, delta: &kg_store::FileDelta) {
...
) -> Vec<kg::agent_policy::AgentPolicyRule> {
    use kg::anno::AnnotationKind;
    let mut out = Vec::new();
    for (_node, refs) in anno.iter() {
        for r in refs {
            if let AnnotationKind::Rule { rule_kind, args } = &r.kind {
                if let Some(Ok(rule)) = kg::agent_policy::parse_rule(rule_kind, args) {
                    out.push(rule);
                }
            }
        }
...
fn build_work_item(
    ...
    for edge in &edges {
        ...
        let mut anchor_payload: Option<kg::anno::WorkItemAnno> = None;
...
fn last_modified_for(rig_root: &Path, id: &str, anchors: &[WorkItemAnchor]) -> u64 {
    ...
    if let Some(anchor) = anchors.first() {
        if let Some(at) = anchor.anno.at.as_deref() {
            if let Some(secs) = kg::timefmt::parse_rfc3339(at) {
                return secs;
            }
        }
