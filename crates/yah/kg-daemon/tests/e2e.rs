...
use kg::anno::{AnnotationKind, TicketStatus};
use kg::edge::EdgeKind;
use kg::event::{ArchEvent, FileEvent, FileEventKind, IndexReason};
...
use kg_daemon::{default_snapshot_path, DaemonError, KgService};
...
"#,
    ...
    let ticket_id = kg::ids::NodeId::compute(
        Lang::Rust,
        "ticket:R042-T1",
        "<work-item>",
    );
...
async fn ticket_prompt_renders_pickup_via_shared_renderer() {
    ...
    // <id> --prompt` both flow through `kg::prompt::render`, so for a
...
async fn assemble_prelude_builds_a_cached_prefix_for_known_ticket() {
    ...
    // SDK pane injects on every turn. Backed by kg::prelude::assemble
...
async fn move_ticket_writes_yah_at_timestamp() {
    ...
    assert!(
        kg::timefmt::parse_rfc3339(val).is_some(),
...
async fn archive_ticket_strips_yah_lines_and_writes_event_shard() {
    ...
    let path = write(
        ...
        "//! @yah:relay(R901, \"to-archive\")\n\
         //! @yah:status(review)\n\
         //! @arch:see(.yah/arch/authored/foo.md)\n\
         //! preserved doc text\n",
    );
