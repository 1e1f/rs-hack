...
//!
//! [`WorkItem`] is the wire DTO the daemon ships from
...
/// own view. The board-recompute layer (this module) consumes the
...
pub struct WorkItemAnchor {
    ...
    /// the per-anchor copies are what this module compares to detect
    /// field disagreements.
    #[serde(default)]
    pub anno: WorkItemAnno,
}

/// Wire shape for one synthetic Relay/Ticket as seen from the board /
/// RPC layer. Combines the synthetic node's identity (`node`, `id`,
/// `item_type`), the parsed annotation payload (`anno`), and the
/// structural anchors (`anchors`) the work-item lives on. Used uniformly
/// by `arch.list_tickets`, `arch.list_relays`, and `arch.get_ticket`.
///
