...
/// Direction filter for graph traversal — used by `Store::neighbors` and
/// the `arch.neighbors` RPC. Lives in `kg::edge` because it's a property
/// of edges (graph-traversal direction), not of any one transport.
