...
use std::path::Path;
use kg::anno::{AnnotationKind, TagRef, TicketStatus};
use kg::edge::EdgeKind;
use kg::indexer::LanguageIndexer;
use kg::kind::{CommonKind, NodeKind};
use rpc::Direction;
use kg_anno::{apply_pass, AnnotationIndex};
...
fn lookup_label(store: &Store, label: &str) -> Option<kg::ids::NodeId> {
    store
        .all_node_refs()
...
fn tag_creates_synthetic_tag_nodes_and_tag_edges() {
    ...
    let tag_nodes: Vec<&kg::ids::NodeRef> = store
        .all_node_refs()
...
fn malformed_tag_does_not_attach() {
    ...
    let tags: Vec<&kg::anno::AnnotationRef> = idx
        .get(malformed)
        .iter()
...
fn build_with_tickets() -> (Store, AnnotationIndex, kg_anno::ApplySummary) {
    let mut store = Store::new();
    {
        let mut sink = StoreSink::new(&mut store);
        RustIndexer::new()
...
fn lookup_qualified(store: &Store, qualified: &str) -> Option<kg::ids::NodeId> {
    store
        .all_node_refs()
...
fn orphan_synthetic_tag_node_is_gced_when_last_tag_edge_disappears() {
    use kg_store::{reindex_file, IndexerRegistry};
    ...
    let mut store = Store::new();
    kg_store::walk_and_index(dir.path(), &mut store, &registry).unwrap();
    let mut idx = AnnotationIndex::new();
    apply_pass(&mut store, &mut idx, None);
