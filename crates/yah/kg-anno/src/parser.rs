...
use kg::agent_policy;
use kg::anno::{EngineRef, TagRef, ThinkBudget, TicketStatus, WorkItemAnno};
...
impl WorkItemBuilder {
    ...
    fn apply_modifier(&mut self, kind: &str, body: &str) -> Result<(), String> {
        ...
        match kind {
            ...
            "at" => {
                if kg::timefmt::parse_rfc3339(&stripped).is_none() {
                    return Err(format!(
...
@yah:rule(agent-dont: "Land code without sign-off.")"#;
        ...
        assert_eq!(
            ...
            kg::agent_policy::AgentPolicyKind::Dont
        );
    }
