...
//! @arch:role(extract)
//!
//! `yah-kg-anno-wasm` — wasm-bindgen target on top of [`kg_anno::parser`].
//!
//! The Files-tab KG-overlay extension (see
//! `.yah/arch/authored/yah-files-tab.md`) loads this module to surface live
...
//! [`kg_anno::parse_doc`]) and returns a JS object with two arrays:
//!
//! ```text
...
//! [`kg_anno::ParsedAnnotation`]; `WireError` is a thin DTO around
//! [`kg_anno::ParseError`] that flattens the enum into named fields
...
use kg_anno::{parse_doc as parse_doc_native, ParseError, ParsedAnnotation};

/// Wire form of [`kg_anno::ParseError`]. The native enum is
...
/// for the wire shape; semantics match [`kg_anno::parse_doc`] exactly
...
mod tests {
    use super::*;
    use kg_anno::{RawAnnotation, WorkItemType};
